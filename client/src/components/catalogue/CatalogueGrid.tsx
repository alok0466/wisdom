...
const CatalogueGrid = () => {
  ...
  // ✅ Filter based on title instead of category
  const filteredItems = activeFilter === "All"
    ? items
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-darkText/70">
            Browse our collection of curated interior designs across different styles and spaces.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="aspect-[3/4] bg-gray-200 animate-pulse rounded-md"></div>
            ))
          ) : displayedItems.length > 0 ? (
            displayedItems.map((item) => (
              <CatalogueItem key={item.id} item={item} />
